...
class SubscribeHelper extends AppHelper {
    ...
    function build($options = array()) {
        ...
        $title = $this->Html->tag("h2", __($options["subscribe_title"], true), 
    ...
    function build_form($options) {
        ...
        $indicator = $this->Html->div("", $this->Html->image("/urg_subscription/img/loading.gif"),
                                      array("id" => "subscription-indicator",
                                            "style" => "display: none"));
        $help = $this->Html->div("", __("Press enter to subscribe.", true), 
        ...
        $form .= $this->Form->input("Subscription.email", 
                array("label" => false,
                      ...
                      "placeholder" => __("Please enter email address", true),
                      "autocomplete" => "off"));
    ...
    function dialog_js($options) {
        $js = "$('#subscription-form-$options[subscribe_group_id]').dialog({
                autoOpen: false,
                modal: true,
                title: '" . __($options["subscribe_title"], true) . "',
                buttons: {
                    '" . __("Subscribe now", true) . "' : function() {
